...
def blendImagePair(warped_image, image_2, point):
    ...
    # blend imagees
    RANGE = 10
    output_image[np.int(point[1]):np.int(point[1]) + image_2.shape[0],
                 np.int(point[0]):np.int(point[0]) + image_2.shape[1]] = image_2

    mask_img = np.zeros(output_image.shape, dtype=np.uint8)
    mask_img[np.int(point[1])+RANGE:np.int(point[1]) + image_2.shape[0]-RANGE,
             np.int(point[0])+RANGE:np.int(point[0]) + image_2.shape[1]-RANGE] = 255
