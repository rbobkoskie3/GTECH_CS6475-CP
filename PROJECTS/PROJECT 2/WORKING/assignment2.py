...
def numberOfPixels(image):
    ...
    numPixel = int(image.size)
    #print image.shape
    #print "numPixel:", type(numPixel), numPixel
...
def averagePixel(image):
    ...
    """
    # WRITE YOUR CODE HERE.
    avePixel = int(np.sum(image) / numberOfPixels(image))
    #print "avePixel:", type(avePixel), avePixel
...
def convertToBlackAndWhite(image):
    ...
    for x in range(0, bw_image.shape[0]):
        for y in range(0, bw_image.shape[1]):
            pix = bw_image.item(x, y)
            if pix > 128:
                bw_image.itemset((x, y), 255)
            else:
                bw_image.itemset((x, y), 0)
...
def averageTwoImages(image1, image2):
    ...
    if (image1.shape == image2.shape):
        #average_im = (image1 + image2) / 2 #Note overflow not addressed, ave image is garbled
        average_im = (resize_img/2 + image2/2).astype('uint8')   #overflow addressed by dividing before sumation
    else:
        if (image1.shape > image2.shape):
            resize_img = cv2.resize(image2, (image1.shape[1], image1.shape[0]))
            #average_im = (image1 + resize_img) / 2   #Note overflow not addressed, ave image is garbled
            #average_im = ((image1.astype('uint16') + image2.astype('uint16'))/2).astype('uint8')   #overflow addressed by converting to uint16
            average_im = (image1/2 + image2/2).astype('uint8')   #overflow addressed by dividing before sumation
        else:
            resize_img = cv2.resize(image1, (image2.shape[1], image2.shape[0]))
            #average_im = (resize_img + image2) / 2   #Note overflow not addressed, ave image is garbled
            #average_im = ((resize_img.astype('uint16') + image2.astype('uint16'))/2).astype('uint8')   #overflow addressed by converting to uint16
            average_im = (resize_img/2 + image2/2).astype('uint8')   #overflow addressed by dividing before sumation

    #cv2.imshow('AVE IMAGE', average_im)
    #cv2.waitKey()
    cv2.imwrite('AVE_IMAGE_NO_OVERFLOW.JPG', average_im)
...
def flipHorizontal(image):
    ...
    # WRITE YOUR CODE HERE.
    hrzFlp_image = image
    hrzFlp_image = cv2.flip(image, 1)

    '''
    hrzFlp_image = np.zeros((image.shape[0],image.shape[1]), np.uint8)
    for x in range(0, image.shape[0]):
        for y in range(0, image.shape[1]):
            pix = image.item(x, y)
            #print x, y, (image.shape[1] - y - 1)
            hrzFlp_image.itemset((x, (image.shape[1] - y - 1)), pix)
    '''

    #cv2.imshow('hrzFlp IMAGE', hrzFlp_image)
    #cv2.waitKey()
    cv2.imwrite('HFLIP_IMAGE.JPG', hrzFlp_image)
